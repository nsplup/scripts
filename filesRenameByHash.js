const { readFileSync, readdirSync, renameSync } = require('fs')
...
for (let i = 0, len = files.length; i < len; i++) {
	...
	try {
		...
		const hash = createHash('sha256').update(file).digest('hex').toUpperCase()
		const newName = path.resolve(dirPath, hash + path.parse(n).ext)
		if (oldName !== newName) {
